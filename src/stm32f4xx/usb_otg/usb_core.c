...
USB_OTG_STS USB_OTG_FlushTxFifo (USB_OTG_CORE_HANDLE *pdev , uint32_t num )
{
  ...
  greset.d32 = 0;
  greset.b.txfflsh = 1;
  greset.b.txfnum  = (uint8_t)(num & 0x1f);
  USB_OTG_WRITE_REG32( &pdev->regs.GREGS->GRSTCTL, greset.d32 );
  do
  {
    greset.d32 = USB_OTG_READ_REG32( &pdev->regs.GREGS->GRSTCTL);
...
void USB_OTG_InitDevSpeed(USB_OTG_CORE_HANDLE *pdev , uint8_t speed)
{
  ...
  dcfg.b.devspd = (uint8_t)(speed & 0x3);
...
USB_OTG_STS USB_OTG_CoreInitDev (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  for (i = 0; i <  pdev->cfg.dev_endpoints; i++)
  {
    USB_OTG_DEPCTL_TypeDef  tmp;
    tmp.d32 = USB_OTG_READ_REG32(&pdev->regs.OUTEP_REGS[i]->DOEPCTL);
    if (tmp.b.epena)
    {
      tmp.d32 = 0;
      tmp.b.epdis = 1;
    ...
    USB_OTG_WRITE_REG32( &pdev->regs.OUTEP_REGS[i]->DOEPCTL, tmp.d32);
...
USB_OTG_STS USB_OTG_EPActivate(USB_OTG_CORE_HANDLE *pdev , USB_OTG_EP *ep)
{
  ...
  * register. */
  ...
  {
    depctl.b.mps    = ep->maxpacket & 0x7ff;
    depctl.b.eptype = (uint8_t)(ep->type & 0x3);
    depctl.b.txfnum = (uint8_t)(ep->tx_fifo_num & 0xf);
    depctl.b.setd0pid = 1;
    depctl.b.usbactep = 1;
...
USB_OTG_STS USB_OTG_EPStartXfer(USB_OTG_CORE_HANDLE *pdev , USB_OTG_EP *ep)
{
  ...
  if (ep->is_in == 1)
  {
    ...
    else
    {
      ...
      * exist ? 1 : 0)
      */
      deptsiz.b.xfersize = ep->xfer_len & 0x7ffff;
      deptsiz.b.pktcnt = ((ep->xfer_len - 1 + ep->maxpacket) / ep->maxpacket) & 0x3ff;
  ...
  else
  {
    ...
    */
    if (ep->xfer_len == 0)
    {
      deptsiz.b.xfersize = ep->maxpacket & 0x7ffff;
      deptsiz.b.pktcnt = 1;
    }
    else
    {
      deptsiz.b.pktcnt = ((ep->xfer_len + (ep->maxpacket - 1)) / ep->maxpacket) & 0x3ff;
      deptsiz.b.xfersize = (deptsiz.b.pktcnt * ep->maxpacket) & 0x7ffff;
    }
  ...
  if (ep->is_in == 1)
  {
    ...
    else
    {
      if (ep->xfer_len > ep->maxpacket)
      {
        ...
        deptsiz.b.xfersize = ep->maxpacket & 0x7f;
      }
      else
      {
        deptsiz.b.xfersize = ep->xfer_len & 0x7f;
      }
      deptsiz.b.pktcnt = 1;
    }
    ...
    if (pdev->cfg.dma_enable == 0)
    {
      ...
      {
        {
          fifoemptymsk = (uint32_t)(fifoemptymsk | (uint32_t)(1 << ep->num));
          USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, 0, fifoemptymsk);
        }
      }
    }
