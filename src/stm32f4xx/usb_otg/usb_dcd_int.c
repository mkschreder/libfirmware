...
/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx/usb_otg/usb_dcd_int.h"
...
static uint32_t DCD_HandleInEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    ...
    {
      diepint.d32 = DCD_ReadDevInEP(pdev , (uint8_t)epnum); /* Get In ITR status */
      if ( diepint.b.xfercompl )
      {
        ...
        /* TX COMPLETE */
        USBD_DCD_INT_fops->DataInStage(pdev , (uint8_t)epnum);
...
static uint32_t DCD_HandleOutEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1)
    {
      ...
      doepint.d32 = USB_OTG_ReadDevOutEP_itr(pdev, (uint8_t)epnum);
      ...
      if ( doepint.b.xfercompl )
      {
        ...
        /* Inform upper layer: data ready */
        /* RX COMPLETE */
        USBD_DCD_INT_fops->DataOutStage(pdev , (uint8_t)epnum);
