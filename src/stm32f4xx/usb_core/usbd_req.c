...
#include "stm32f4xx/usb_core/usbd_ioreq.h"
...
static void USBD_GetDescriptor(USB_OTG_CORE_HANDLE *pdev, USB_SETUP_REQ *req) {
	...
	switch(req->wValue >> 8) {
		...
		case USB_DESC_TYPE_CONFIGURATION:
			pbuf =
			    (uint8_t *)pdev->dev.class_cb->GetConfigDescriptor(pdev->cfg.speed, &len);
#ifdef USB_OTG_HS_CORE
			if((pdev->cfg.speed == USB_OTG_SPEED_FULL) &&
			   (pdev->cfg.phy_itface == USB_OTG_ULPI_PHY)) {
				pbuf = (uint8_t *)pdev->dev.class_cb->GetOtherConfigDescriptor(
				    pdev->cfg.speed, &len);
			}
#endif
		...
		case USB_DESC_TYPE_STRING:
			switch((uint8_t)(req->wValue)) {
				...
				case USBD_IDX_MFC_STR:
					pbuf = pdev->dev.usr_device->GetManufacturerStrDescriptor(pdev->cfg.speed,
					                                                          &len);
					break;

				case USBD_IDX_PRODUCT_STR:
					pbuf =
					    pdev->dev.usr_device->GetProductStrDescriptor(pdev->cfg.speed, &len);
					break;
				...
				case USBD_IDX_CONFIG_STR:
					pbuf = pdev->dev.usr_device->GetConfigurationStrDescriptor(pdev->cfg.speed,
					                                                           &len);
					break;

				case USBD_IDX_INTERFACE_STR:
					pbuf =
					    pdev->dev.usr_device->GetInterfaceStrDescriptor(pdev->cfg.speed, &len);
					break;

				default:
#ifdef USB_SUPPORT_USER_STRING_DESC
					pbuf = pdev->dev.class_cb->GetUsrStrDescriptor(pdev->cfg.speed,
					                                               (req->wValue), &len);
					break;
#else
					USBD_CtlError(pdev, req);
		...
		case USB_DESC_TYPE_DEVICE_QUALIFIER:
			...
			if(pdev->cfg.speed == USB_OTG_SPEED_HIGH) {
				...
				pbuf = (uint8_t *)pdev->dev.class_cb->GetConfigDescriptor(pdev->cfg.speed,
				                                                          &len);
