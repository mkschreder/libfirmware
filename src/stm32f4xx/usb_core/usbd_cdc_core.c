/**
  ...
  *             - Enumeration as CDC device with 2 data endpoints (IN and OUT) and 1
  command endpoint (IN)
  ...
  *           @note
  *             For the Abstract Control Model, this core allows only transmitting
  the requests to
  *             lower layer dispatcher (ie. usbd_cdc_vcp.c/.h) which should manage
  each request and
  ...
  *            This driver doesn't implement the following aspects of the
  specification
  *            (but it is possible to manage these features with some modifications
  on this driver):
  *             - Any class-specific aspect relative to communication classes should
  be managed by user application.
...
/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx/usb_core/usbd_cdc_core.h"
#include "stm32f4xx/usb_core/usbd_req.h"
#include "stm32f4xx/usb_vcp/usbd_desc.h"
...
__ALIGN_BEGIN uint8_t usbd_cdc_OtherCfgDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END = {
    ...
    0x03,                             /* bmAttributes: Interrupt */
    LOBYTE(CDC_CMD_PACKET_SZE),       /* wMaxPacketSize: */
    HIBYTE(CDC_CMD_PACKET_SZE), 0xFF, /* bInterval: */
...
static uint8_t usbd_cdc_Init(void *pdev, uint8_t cfgidx) {
	...
	DCD_EP_PrepareRx(pdev, CDC_OUT_EP, (uint8_t *)(USB_Rx_Buffer),
...
static uint8_t usbd_cdc_DataIn(void *pdev, uint8_t epnum) {
	...
	if(USB_Tx_State == 1) {
		if(APP_Rx_length == 0) {
			...
			DCD_EP_Tx(pdev, CDC_IN_EP, (uint8_t *)&APP_Rx_Buffer[USB_Tx_ptr],
			          USB_Tx_length);
		}
	}
...
#ifdef USB_OTG_HS_INTERNAL_DMA_ENABLED
		APP_Rx_length = (uint32_t)(APP_Rx_length & ~0x03);
#endif /* USB_OTG_HS_INTERNAL_DMA_ENABLED */
