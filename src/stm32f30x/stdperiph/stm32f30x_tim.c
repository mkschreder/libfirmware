...
#else
	uint32_t x = TIMx->CCER;
	x &= ~((uint32_t)CCER_CCE_SET << (uint32_t)TIM_Channel);
	x |=  ((uint32_t)TIM_CCx << (uint32_t)TIM_Channel);
	TIMx->CCER = x;
#endif
}
...
#else
	uint32_t x = TIMx->CCER;
	x &= ~((uint32_t)CCER_CCNE_SET << (uint32_t)TIM_Channel);
	x |=  ((uint32_t)TIM_CCxN << (uint32_t)TIM_Channel);
	TIMx->CCER = x;
#endif
}
/**
