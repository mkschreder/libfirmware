...
void TIM_SetRepetitionCounter(TIM_TypeDef *TIMx, uint16_t rcr){
	TIMx->RCR = rcr;
}
                                ...
                                uint16_t TIM_IC1Polarity, uint16_t TIM_IC2Polarity)
{
  ...
  //tmpccmr1 &= (uint16_t)(((uint16_t)~((uint16_t)TIM_CCMR1_IC1F)) & ((uint16_t)~((uint16_t)TIM_CCMR1_IC2F)));
...
#else
	// ref Martin Schröder
	// this is modified version because STs default was always disabling and then reenabling the channel (wtf?) causing a ripple on repeated calls
	if(TIM_CCx == TIM_CCx_Disable)
		TIMx->CCER &= (uint16_t)~(uint16_t)(CCER_CCE_Set << TIM_Channel);
	else if(TIM_CCx == TIM_CCx_Enable)
		TIMx->CCER |=  (uint16_t)(TIM_CCx << TIM_Channel);
#endif
}
