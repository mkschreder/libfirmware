...
volatile USBLIB_EPBuf *EPBufTable = (USBLIB_EPBuf*)USB_PBUFFER;
...
void USBLIB_Reset(void)
{
    ...
    for (uint8_t i = 0; i < EPCOUNT; i++) {
        ...
        Addr = (uint16_t)(Addr + EpData[i].TX_Max);
        EPBufTable[i].RX_Address.Value = Addr;
        if (EpData[i].RX_Max >= 64)
            EPBufTable[i].RX_Count.Value = 0x8000 | (uint16_t)((EpData[i].RX_Max / 64) << 10);
        else
            EPBufTable[i].RX_Count.Value = (uint16_t)((EpData[i].RX_Max / 2) << 10);

        Addr = (uint16_t)(Addr + EpData[i].RX_Max);

        if (!EpData[i].pRX_BUFF)
            EpData[i].pRX_BUFF = (uint16_t *)kzmalloc(EpData[i].RX_Max);

        USB->EPR[i] = (EpData[i].Number | EpData[i].Type | RX_NAK | TX_NAK);
    }

/*
    for (uint8_t i = EPCOUNT; i < 8; i++) {
        USB->EPR[i] = (uint32_t)(i | RX_NAK | TX_NAK);
    }
*/
...
{
	if(EPn >= EPCOUNT) return;
    register uint16_t val = (uint16_t)USB->EPR[EPn];
...
void USBLIB_Pma2EPBuf2(uint8_t EPn){
    uint32_t Count = EpData[EPn].lRX = (EPBufTable[EPn].RX_Count.Value & 0x3FF);
    uint32_t *Address                 = (uint32_t *)(USB_PBUFFER + (uint32_t)(EPBufTable[EPn].RX_Address.Value * 2));
    uint16_t *Distination             = (uint16_t *)EpData[EPn].pRX_BUFF;
    for (uint8_t i = 0; i < Count; i++) {
        *(volatile uint16_t *)Distination = *(volatile uint16_t *)Address;
        Distination++;
        Address++;
    }
	return count;
...
void USBLIB_EPBuf2Pma(uint8_t EPn)
{
    ...
    EPBufTable[EPn].TX_Count.Value = (uint16_t)Count;

    Distination = (uint32_t *)(USB_PBUFFER + (uint32_t)(EPBufTable[EPn].TX_Address.Value * 2));
...
{
	uint16_t dummy = 0xdead;
    EpData[EPn].lTX      = Length;
    EpData[EPn].pTX_BUFF = (Data)?Data:&dummy;
    if (Length > 0) {
...
{
    switch (SPacket->wValueH) {
    case USB_DEVICE_DESC_TYPE:
        USBLIB_SendData(0, (uint16_t *)&USB_DEVICE_DESC[0], 18);
        break;
    case USB_CFG_DESC_TYPE:
        USBLIB_SendData(0, (uint16_t *)&USBD_CDC_CFG_DESCRIPTOR[0], sizeof(USBD_CDC_CFG_DESCRIPTOR));
        break;

    case USB_DEVICE_QR_DESC_TYPE:
        USBLIB_SendData(0, (uint16_t *)&USB_DEVICE_QR_DESC[0], sizeof(USB_DEVICE_QR_DESC));
        break;
    ...
    case USB_STR_DESC_TYPE: {
        ...
        if(SPacket->wValueL >= (sizeof(desc) / sizeof(desc[0])))  {
            USBLIB_SendData(0, (uint16_t*)&desc[1], 6);
        } else {
            const struct descriptor *pSTR;
            pSTR = &desc[SPacket->wValueL];
...
void USBLIB_EPHandler(uint16_t Status)
{
    ...
    if (EP & EP_CTR_RX && EPn < EPCOUNT) { //something received
        ...
        if (EPn == 0) { //If control endpoint
            if (EP & USB_EP0R_SETUP) {
				...
				case USB_REQUEST_SET_ADDRESS:
					DeviceAddress = (uint8_t)(SetupPacket->wValueL);
					//USBLIB_SendData(0, &DeviceStatus, 2);
					USBLIB_SendData(0, 0, 0);
					break;
				case USB_REQUEST_GET_STATUS:
					USBLIB_SendData(0, &DeviceStatus, 2);
					//USBLIB_SendData(0, (uint16_t *)&USBD_CDC_CFG_DESCRIPTOR[0], 9);
					break;
...
*/
			...
			//toggle |= EP_DTOG_RX | EP_STAT_RX | EP_SETUP | EP_EA;
        	//USB->EPR[EPn] &= EP_DTOG_RX | EP_STAT_RX | EP_SETUP | EP_EA; //0x78f;
        ...
        //USB->EPR[EPn] &= 0x78f;
      	USB->EPR[EPn] &= EP_TYPE | EP_KIND | EP_CTR_TX | EP_EA; //0x78f;
        USBLIB_setStatRx(EPn, RX_VALID);
    }
	if (EP & EP_CTR_TX && EPn < EPCOUNT) { //something transmitted
        if (DeviceAddress) {
            USB->DADDR    = DeviceAddress | 0x80;
            DeviceAddress = 0;
        }

        //USB->EPR[EPn] &= (EP_TYPE | EP_KIND | EP_EA);

		// data still in buffer to be transmitted
        if (EpData[EPn].lTX) {
            USBLIB_EPBuf2Pma(EPn);
            USBLIB_setStatTx(EPn, TX_VALID);
        }

        USB->EPR[EPn] &= (EP_CTR_RX | EP_TYPE | EP_KIND | EP_EA); //0x870f
    }
}
...
void USB_LP_CAN1_RX0_IRQHandler()
{
    if (USB->ISTR & USB_ISTR_RESET) { // Reset
        USB->ISTR &= (uint32_t)~USB_ISTR_RESET;
        USBLIB_Reset();
        //return;
    }
    if (USB->ISTR & USB_ISTR_CTR) { //Handle data on EP
        USB->ISTR &= (uint32_t)~USB_ISTR_CTR;
    ...
    if (USB->ISTR & USB_ISTR_SUSP) {
        ...
        if (USB->DADDR & 0x7f) {
            USB->DADDR = 0;
            USB->CNTR &= (uint32_t)~USB_CNTR_SUSPM;
        }
        //return;
    }
