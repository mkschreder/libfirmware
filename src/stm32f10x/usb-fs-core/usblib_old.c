...
USBLIB_EPBuf *EPBufTable = (USBLIB_EPBuf*)USB_PBUFFER;
volatile USBLIB_SetupPacket *  SetupPacket;
...
{
    register uint32_t val = USB->EPR[EPn];
...
void USBLIB_GetDescriptor(volatile USBLIB_SetupPacket *SPacket)
{
    ...
    case USB_STR_DESC_TYPE: {
        ...
        if(SPacket->wValue.L >= (sizeof(desc) / sizeof(desc[0])))  {
            USBLIB_SendData(0, (uint16_t*)&desc[1], 6);
        } else {
            const struct descriptor *pSTR;
            pSTR = &desc[SPacket->wValue.L];
...
void USBLIB_EPHandler(uint32_t Status)
{
        ...
        if (EPn == 0) { //If control endpoint
            if (EP & USB_EP0R_SETUP) {
                ...
                case USB_REQUEST_SET_ADDRESS:
                    DeviceAddress = (uint8_t)(SetupPacket->wValue.L);
                    USBLIB_SendData(0, 0, 0);
                    break;
    ...
    if (EP & EP_CTR_TX) { //something transmitted
        ...
        uint32_t keep = USB->EPR[EPn] & (EP_TYPE | EP_KIND | EP_EA);
        USB->EPR[EPn] = keep; //EP_CTR_RX | EP_TYPE | EP_KIND | EP_EA;
...
void USB_LP_CAN1_RX0_IRQHandler()
{
    ...
    if (USB->ISTR & USB_ISTR_CTR) { //Handle data on EP
        USBLIB_EPHandler((uint32_t)USB->ISTR);
    ...
    if (USB->ISTR & USB_ISTR_SUSP) {
        USB->ISTR &= (uint32_t)~USB_ISTR_SUSP;
        if (USB->DADDR & 0x7f) {
            USB->DADDR = 0;
